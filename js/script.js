...
function htmlProducts(data) {
    ...
    productList.forEach((product) => {
        ...
        <div class="productPrice"> $ ${product.price}.00 </div> 
        <button class="buyButton"> Comprar </button>
        `

        const installmentsDiv = document.createElement("div")
        installmentsDiv.classList.add("installmentsDiv")
        installmentsDiv.style.display = "none"
        productDiv.insertBefore(installmentsDiv, productDiv.children[4])
        installmentsDiv.innerHTML = `<span class="installmentSpan"> ou ${product.installments.count}x <br> de R$${product.installments.value} </span> `
        installmentsDiv.addEventListener("click", function () {
            productDiv.children[5].style.display = "inline"
            productDiv.children[6].style.display = "block"
            productDiv.children[7].style.display = "inline"
            installmentsDiv.style.display = "none"
        })
        ...
        buttonInstallment.addEventListener("click", function () {
            productDiv.children[5].style.display = "none"
            productDiv.children[6].style.display = "none"
            productDiv.children[7].style.display = "none"
...
function loadMore() {
    page++
    urlLoad(page).then(htmlProducts)
}

window.onload = (e) => e.preventDefault()
urlLoad(1).then(htmlProducts).catch(errorEvt)
